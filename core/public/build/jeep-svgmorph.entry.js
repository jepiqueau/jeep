...
const splitCubicBezier = (cB, z) => {
    ...
    if (r1 && r2) {
        ...
        let ptEnd = cB[0].addMPoint(cB[3].substractMPoint(cB[0]).multNumber(z));
        let pt1 = cB[0].addMPoint(ptEnd.substractMPoint(cB[0]).multNumber(1.0 / 3.0));
        let pt2 = cB[0].addMPoint(ptEnd.substractMPoint(cB[0]).multNumber(2.0 / 3.0));
...
const getCubicBezierFromMPoints = (points) => {
    let d = "";
...
const initCurveMPoints = (P1, P2, P3, P4) => {
    let pts = [];
    pts = [...pts, P1];
    pts = [...pts, P2];
    pts = [...pts, P3];
...
const permuteCubicBezier = (points, startIndex) => {
    ...
    let cBz = getCubicBezierFromMPoints(newPoints);
    return cBz;
};
...
const alignSegmentPath = (cB, ratio) => {
    ...
    if (ratio > 1.0) {
        ...
        for (let i = 0; i < points.length - 1; i += 3) {
            ...
            let tmpPt = initCurveMPoints(points[i], points[i + 1], points[i + 2], points[i + 3]);
            while (nbSplit > 1) {
                let z = 1.0 / nbSplit;
                let pts = initCurveMPoints(tmpPt[0], tmpPt[1], tmpPt[2], tmpPt[3]);
...
const lineToCubicBezier = (cBezier, oPath, letter) => {
    let cBz = cBezier;
    let tPoint = new MPoint();
    let nPoint = new MPoint();
    if (letter) {
...
const quadraticToCubicBezier = (cBezier, oPath, letter) => {
    ...
    cBz.cBz = cBz.cBz.concat('C ');
    const cP1 = cBz.lPoint.addMPoint(nP2.substractMPoint(cBz.lPoint).multNumber(2.0 / 3.0));
    cBz.cBz = cBz.cBz.concat(cP1.toFixed(2) + " ");
    const cP2 = nP3.addMPoint(nP2.substractMPoint(nP3).multNumber(2.0 / 3.0));
...
const JeepSvgmorph = class {
    ...
    render() {
        ...
        if (this.isSVG) {
            toRender = [...toRender,
                h("div", { id: "morph-container" }, h("div", { id: "morph-svg-container" }, h("svg", { id: "morph-svg", width: "500", height: "500", viewBox: "0 0 500 500", xmlns: "http://www.w3.org/2000/svg" }, h("path", { id: "initPath" }, h("animate", { id: "animPath", begin: "initPath.click" }), this._fillColor.length > 1 && !this._uniqueColor
